...
export const VehiculoDetailDialog = ({ open, onOpenChange, vehiculo, loading, error }: Props) => {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <Button variant="outline" onClick={() => vehiculo && onOpenChange(true)}>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-2 text-gray-600">Cargando detalles del vehículo...</span>
          </div>
        </DialogContent>
      </Dialog>
    );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="shadow-xl border border-border rounded-xl max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-gray-900">{vehiculo.tipo?.marca || vehiculo.brand || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Modelo</Label>
                    <p className="text-sm text-gray-900">{vehiculo.tipo?.modelo || vehiculo.model || 'N/A'}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Año</Label>
                    <p className="text-sm text-gray-900">{vehiculo.tipo?.año || vehiculo.manufacturingYear || 'N/A'}</p>
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Categoría</Label>
                    <p className="text-sm text-gray-900">{vehiculo.tipo?.categoria || 'N/A'}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-1">
                  <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide">DNI</Label>
                ...
                {vehiculo.propietario?.nombres && vehiculo.propietario?.apellidos && (
                  <div className="pt-2 border-t border-gray-100">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Nombres</Label>
                        <p className="text-sm text-gray-700">{vehiculo.propietario.nombres}</p>
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Apellidos</Label>
                        <p className="text-sm text-gray-700">{vehiculo.propietario.apellidos}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <Card className="shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <CardContent className="space-y-3">
                ...
                {vehiculo.empresa?.estado && (
                  <div className="space-y-1">
                    <Label className="text-xs font-medium text-gray-500 uppercase tracking-wide">Estado Empresa</Label>
                    <Badge 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {vehiculo.tipo?.vehicleInfo && (
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-gray-900">{vehiculo.tipo.vehicleInfo}</p>
                    </div>
                  )}
                  {vehiculo.tipo?.id && (
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-mono text-gray-900">{vehiculo.tipo.id}</p>
                    </div>
                  )}
                </div>
              </CardContent>
